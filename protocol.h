...
struct uscVariables
{
    ...
    // offset: 6
    uint16_t buffer[3]; // protects other RAM from being corrupted by improper instructions
    ...
    // offset: 96
    uint16_t scriptDone; // 1 = done; 2 = about to run a single step then be done - placed here to protect against accidental overwriting of servoSetting

    // offset: 97
    uint16_t buffer2; // protects other RAM from being corrupted by improper instructions
